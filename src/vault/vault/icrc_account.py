...
def encodeIcrcAccount(owner: Principal, subaccount: bytes = None) -> str:
    ...
    if (
        subaccount is None
        or len(subaccount) == 0
        or (len(subaccount) == 32 and all(b == 0 for b in subaccount))
    ):
        return str(owner)
...
def compute_crc(owner: Principal, subaccount: bytes) -> str:
    ...
    checksum_base32 = (
        base64.b32encode(checksum_bytes).decode("utf-8").lower().rstrip("=")
    )
...
def decodeIcrcAccount(text: str) -> tuple[Principal, bytes]:
    ...
    if "." not in text:
        return Principal.from_str(text), bytes(
            32
        )  # Return 32 zero bytes for default subaccount
