...
def test_initial_test_mode_status():
    ...
    try:
        ...
        if test_mode_enabled is None or tx_id is None:
            print_error(f"Missing test mode fields: enabled={test_mode_enabled}, tx_id={tx_id}")
            return False
        ...
        print_error(f"Error testing initial test mode status: {e}\n{traceback.format_exc()}")
        return False
...
def test_deploy_vault_with_test_mode_enabled():
    ...
    try:
        ...
        print_error(f"Error testing vault deployment with test mode: {e}\n{traceback.format_exc()}")
        return False
...
def test_deploy_vault_with_test_mode_disabled():
    ...
    try:
        ...
        print_error(f"Error testing vault deployment without test mode: {e}\n{traceback.format_exc()}")
        return False
...
def test_mock_transfer_in_test_mode():
    ...
    try:
        ...
        if new_tx_id != initial_tx_id + 1:
            print_error(f"Expected tx_id to increment from {initial_tx_id} to {initial_tx_id + 1}, but got {new_tx_id}")
            return False
        ...
        print_error(f"Error testing mock transfer in test mode: {e}\n{traceback.format_exc()}")
        return False
...
def test_transaction_history_skip_in_test_mode():
    ...
    try:
        ...
        print_error(f"Error testing transaction history skip: {e}\n{traceback.format_exc()}")
        return False
