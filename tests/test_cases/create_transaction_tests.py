...
def test_create_mint_transaction():
    ...
    if final_balance == expected_balance:
        print_ok(
            f"Mint transaction created successfully. Balance updated from {initial_balance} to {final_balance}"
        )
        return True
    else:
        print_error(
...
def test_create_transfer_transaction():
    ...
    )  # User balance increases on deposit
    expected_vault_balance = (
        initial_vault_balance + amount
    )  # Vault balance also increases

    if (
        final_user_balance == expected_user_balance
        and final_vault_balance == expected_vault_balance
    ):
        print_ok(
            f"Transfer transaction created successfully. User: {initial_user_balance} -> {final_user_balance}, Vault: {initial_vault_balance} -> {final_vault_balance}"
        )
        return True
    else:
        print_error(
            f"Balance mismatch. User expected {expected_user_balance}, got {final_user_balance}. Vault expected {expected_vault_balance}, got {final_vault_balance}"
        )
        return False
...
def test_create_burn_transaction():
    ...
    if final_balance == expected_balance:
        print_ok(
            f"Burn transaction created successfully. Balance updated from {initial_balance} to {final_balance}"
        )
        return True
    else:
        print_error(
...
def test_transaction_appears_in_history():
    ...
    if final_count == initial_count + 1:
        # Look for our specific transaction (convert to string for comparison)
        found_tx = False
        for tx in final_txs:
            # Transaction IDs are returned as strings with underscores for readability
            tx_id_str = str(tx_id).replace("_", "")
            returned_id_str = str(tx.get("id", "")).replace("_", "")
            if tx_id_str == returned_id_str:
                found_tx = True
                break
        ...
        if found_tx:
            ...
            print_error(
                f"Transaction count increased but specific transaction {tx_id} not found in {[tx.get('id') for tx in final_txs]}"
            )
            return False
    else:
        print_error(
            f"Transaction count mismatch. Expected {initial_count + 1}, got {final_count}"
        )
        return False
