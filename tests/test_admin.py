...
from src.vault.vault.admin import set_admin, reset, set_heartbeat_interval_seconds, set_ledger_canister
from src.vault.vault.entities import app_data, LedgerCanister
...
class TestAdmin(unittest.TestCase):
    ...
    def test_admin_can_set_ledger_canister(self):
        ...
        self.assertEqual(LedgerCanister["new-ledger-canister-id"].principal, "new-ledger-canister-principal")
    ...
    def test_non_admin_cannot_set_ledger_canister(self):
        ...
        app_data_mock = MagicMock()
        app_data_mock.admin_principal = "admin-principal"  # This is different from "non-admin-principal"
        patch('src.vault.vault.admin.app_data', return_value=app_data_mock).start()
        
        # Set up the initial state - ensure the ledger canister exists with the expected value
        # Check if ledger canister already exists, if so, update it, otherwise create it
        try:
            existing_canister = LedgerCanister["new-ledger-canister-id"]
            if existing_canister:
                existing_canister.principal = "adminsss-principal"
        except (KeyError, AttributeError):
            # If it doesn't exist or we get an attribute error, create it
            LedgerCanister(_id="new-ledger-canister-id", principal="adminsss-principal")
        
        # Call set_ledger_canister with non-admin caller and expect exception
        with self.assertRaises(ValueError):
            set_ledger_canister("non-admin-principal", "new-ledger-canister-id", new_principal)
            
        # Verify ledger canister was not changed
        self.assertEqual(LedgerCanister["new-ledger-canister-id"].principal, "adminsss-principal")
