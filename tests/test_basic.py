        ...
        while True:
            try:
                ...
                if not app_data().log_length:
                    ...
                    mock_response = get_transactions(
                        app_data().log_length, TRANSACTION_BATCH_SIZE
                    )
                    logger.debug(
                        f"Sending mock response for get_transactions({app_data().log_length}, {TRANSACTION_BATCH_SIZE})"
                    )
...
async def test_basic():
    ...
    for i in range(3):
        ...
        logger.debug(
            "Database dump: %s" % Database.get_instance().dump_json(pretty=True)
        )
