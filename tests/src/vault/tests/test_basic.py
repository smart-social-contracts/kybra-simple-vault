        ...
        while True:
            try:
                ...
                if not app_data().log_length:
                    ...
                    mock_response = get_transactions(app_data().log_length, 10)  # assuming TRANSACTION_BATCH_SIZE is 10
                    logger.debug(f"Sending mock response for get_transactions({app_data().log_length}, 10)")
